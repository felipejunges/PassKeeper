# PassKeeper

PassKeeper is a proof-of-concept password manager built with .NET 10 and a GTK-based UI. It is designed to run on both Windows and Linux platforms. The main goal of this project is to explore secure password storage techniques in a cross-platform desktop application.

## Project Objective

The objective of PassKeeper is to store users' passwords as securely as possible, using modern encryption and best practices available within the .NET ecosystem. The project demonstrates how to implement password management and encryption in a desktop application.

## Important Disclaimer

**This is NOT a professional or production-ready project.**

- Do NOT trust PassKeeper to store important, sensitive, or critical data.
- The application is intended for educational purposes and as a proof of concept only.
- There may be security flaws or limitations that make it unsuitable for real-world use.

## Contributions & Feedback

We welcome suggestions and feedback, especially regarding how to improve the security of the stored data. If you have ideas or recommendations, please open an issue or submit a pull request.

## Features

- Cross-platform: Runs on Windows and Linux
- GTK UI for a native desktop experience
- Password storage with encryption

## Requirements

- .NET 10 SDK
- GTK libraries (for your platform)

## Getting Started

1. Clone the repository
2. Install the required dependencies
3. Build and run the application using your preferred .NET tools

## License

This project is open source and available under the MIT License.
